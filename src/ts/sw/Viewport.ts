...
export const getAvailSize = ()=>{
    ...
    const w = ((mob ? screen.width  : Math.max(window.innerWidth , screen.availWidth )) || 0) + "px";
    const h = ((mob ? screen.height : Math.max(window.innerHeight, screen.availHeight)) || 0) + "px";
    return {
        "--avail-width": l ? h : w,
        "--avail-height": l ? w : h,
